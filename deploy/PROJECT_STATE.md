...
| `PAPER_PENDING_EXPIRY_HOURS` | `4.0` | how long an untouched paper limit waits for its pullback before being dropped. A fresh suggestion for the same product supersedes the pending one regardless, so this only binds on products that stop being suggested |
...
- [ ] **The two books now diverge by construction, and that is intended.** Paper has no risk budget and no sleeve, so it takes ideas live refuses as `risk_cap` or trims for headroom — `20260902T181231Z_BTC` is the first instance. Paper is a record of *the strategy*; the live book is a record of *the strategy inside a $2,000 sleeve*. Do not present one as a forecast of the other, and do not "fix" the divergence by loosening the live caps
