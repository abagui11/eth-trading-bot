...
- [ ] **BTC is effectively not evaluated by the hourly cycle.** In the 7 days to 2026-09-10, 380 of 383 BTC rows in `suggestions` were the synthetic `"No independent setup for this asset this cycle."` filler that `agent.py` writes for whichever product `propose_trades_multi` did not return a decision for. The multi-asset call is specified as 0–2 trades, but in practice it answers for ETH and stays silent on BTC, so BTC's charts are rendered and paid for every cycle and then discarded. Whether this is prompt framing, the ETH-first ordering, or the corrupted range line fixed on 2026-09-10 is **not yet diagnosed** — the filler is indistinguishable in the ledger from a genuine BTC abstention, which is the first thing to fix if this is investigated (give the filler its own rationale prefix or a `setup_tag`)
- [ ] **The abstention funnel is not observable without parsing prose.** Diagnosing the 2026-09-06 → 09-10 quiet spell required `LIKE '%parse_error%'` against `suggestions.rationale`, because `executed=0` conflates a genuine `no_trade`, a validation rejection, an audit downgrade, and a watchdog shadow. Over that window: 442 rows, 221 never-evaluated BTC fillers, 50 ETH proposals killed by `validate.py` (35 R/R below 1.0, 10 stops tighter than 0.8%, 5 order-block mismatches), and 1 actionable idea — which did fill, as HQ #30. A `reason_code` column on `suggestions`, mirroring `vault_allocations.skip_reason`, would make this a `GROUP BY` instead of a string search. Note the R/R failures are mostly 0.24–0.60 rather than marginal, so the 1.0 floor is not obviously the binding constraint — **do not tune either floor without measuring it against the recorded book first**
