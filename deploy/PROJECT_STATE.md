...
- [ ] **The auto/FIFO path gets exactly one chance per idea, at mint.** The mill calls `maybe_execute_live` only inside the `paper.open_from_idea(...) is not None` branch, and `paper_trades.idea_id` is `UNIQUE`, so an idea that is refused is never re-offered. Combined with the hub-side `book_not_empty` rule, auto only fills in the narrow window where a *brand-new* idea mints **while the sleeve is empty**. When a clip closes, nothing replays the backlog — the sleeve stays idle until the next fresh mint. This weakens the "always have a clip open" objective; a re-offer sweep over recent unfilled ideas on clip close would close the gap
- [ ] **Sleeve sizes are constants, never reconciled against the broker.** `LIVE_MILL_SLEEVE_USD` / `LIVE_HQ_EQUITY_USD` are trusted blindly by the exposure checks; nothing compares them to `get_account_summary()`. Funds sitting in the spot/USDC wallet rather than the CFM futures wallet are invisible to the bot, so clips can be sized against capital the account does not have. `deploy/diagnose_live.py` reports the real futures cash
- [ ] **`/opt/trade-ideas` on the VPS is not a git repo**, so the mill's own changes cannot be deployed with `git pull` and shipped hub-side only. Behaviour stays correct because every real limit is enforced in `execute.execute_mill_idea`, but the mill can't tag its `ideas.live_fill_type`, so auto-vs-manual attribution exists only in the hub ledger
