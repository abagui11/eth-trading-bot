...
| Date | Change |
|---|---|
| 2026-09-01 | **Mill live clips are the same clickable cards as Eva.** The mill tab was a bare table (opened / instrument / side / qty / entry / stop) with no mark, no target, and no R:R. Open and closed mill rows now go through `enrich_live_trades` and the shared `trade_card` accordion — entry, stop, the armed TP, planned or realized R, size, and unrealized P&L. A one-contract mill clip only displays TP1, because that is the only rung `_tp_ladder` actually rests. `/api/trades/live` returns the enriched payload so the 15s mill poll keeps the mark fresh. |
