...
- [ ] **The two books now run different strategies, and that is intended.** Two independent divergences, both from 2026-09-02. **Sizing:** paper has no risk budget and no sleeve, so it takes ideas live refuses as `risk_cap` or trims for headroom (`20260902T181231Z_BTC` is the first instance). **Fill model:** paper waits for the limit and live buys the mark, so paper is now a prospective test of the *limit-order* strategy that the retrospective sweep rejected for live — 47% fill at 1h, and the trades that don't fill are the high-chase ones a market order would have taken. This is the A/B worth having, since the earlier verdict rested on 15 replayed trades, but it means paper is a record of *a different strategy*, not a forecast of live. Do not present one as the other, and do not "fix" the divergence by loosening the live caps. Revisit once paper has ~30 pending-era positions
- [ ] **Live still buys the mark; resting real limit orders remains rejected.** The gateway has no plain limit method (market IOC, stop-limit, bracket only), so it would need `limit_limit_gtc`/`gtd` venue code. The measured objection is stronger than the plumbing one: fills are bimodal — every trade with chase above ~0.2R never returned to the entry within the hour (0 of 8), and every trade with chase under 0.06R filled instantly at the mark, where a market order is already equivalent. So the limit never wins a better price on the trades that matter; it only declines them, making it the same 55–60% trade-count filter as the in-block gate and the chase cap. Most of its apparent edge at 1h (+$21.20 of +$39.84) was skipping 8 losers that also contained 2 of the big winners
