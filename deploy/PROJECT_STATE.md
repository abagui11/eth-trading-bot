...
| `LIVE_HQ_RISK_PCT` | `0.005` | **HQ sizing driver.** One clip is however many whole CDE nanos fit this much risk, measured against the **fill price** not the planned entry. $10 on a $2,000 sleeve sizes ETH at 1–4 nanos (median 2.5 across the first 15 positions). Levels never move to fit; only the clip changes |
...
| `LIVE_MAX_OPEN_HQ` | `2` | skip new HQ live ideas when full. Bounded in practice by `LIVE_MAX_LEVERAGE` — see section 10 |
| `LIVE_MAX_PER_PRODUCT_HQ` | `1` | concurrent HQ positions in one product (was pinned in `vault.policy()`) |
| `LIVE_MAX_LEVERAGE` | `1.0` | total HQ notional ≤ sleeve × this (hard ceiling 2x). The real limit on concurrent positions — see section 10 |
...
- [ ] **HQ's second slot is BTC-only in practice.** Only ETH/BTC are traded and `LIVE_MAX_PER_PRODUCT_HQ` is `1`, so while Eva holds ETH the remaining slot can only be filled by a BTC setup. Not currently costing fills (zero `Vault skip` lines), but `grep 'Vault skip: product_open'` is the signal that it has started to
- [ ] **`LIVE_MAX_OPEN_HQ` is not the binding cap — `LIVE_MAX_LEVERAGE` is.** Total HQ notional may not exceed sleeve × leverage, which at `2000 × 1.0` is $2,000. Under risk-based sizing a clip is $240–$960 for ETH and ~$800 for BTC depending on stop distance, so the worst case for four concurrent positions (two tight-stop ETH at 4 nanos plus two BTC) is ~$3,480 and needs ~1.74x. Raising the open cap to 4 therefore needs ~1.8x (`bot_config` treats 2x as the hard ceiling). Both caps have to move together. Note the clip now trims to the remaining headroom rather than being refused outright, so the symptom of an over-tight leverage cap is small clips, not `heat_cap` skips
- [ ] **BTC does not fit this sleeve — three independent reasons.** (1) A single nano is one ladder rung, so a BTC clip closes fully at TP1 and later targets are never armed (`execute._tp_ladder`); `_armable_tps` keeps the record honest and `validate.compute_risk_reward` already prices R:R off TP1, so nothing is mispriced, but BTC takes full stop risk for TP1-only reward. (2) One clip is ~39% of sleeve notional. (3) Under risk-based sizing one nano risks $5.36–$11.12 depending on stop distance, so it is refused as `risk_cap` whenever that exceeds the $10 budget — 2 of its 5 recorded trades. Replaying the first 15 HQ positions, BTC was 5 of 15 trades and −$33.45 of the −$36.81 total, on 1 win in 5. The sample is far too small to act on; the geometry is not
...
- [ ] **Paper still opens at the planned entry, not the mark.** `_open_position` books `suggestion.entry`, so paper fills at a pullback price that in 9 of 15 recorded HQ cases never traded that cycle. The 2026-09-02 barrier fix closed the exit-side analogue of this; the entry side remains. It also means the stop study's R-multiples are computed on entries live cannot obtain, so its baseline is optimistic for live even after the stop correction. Risk-based sizing bounds the *dollar* consequence (a worse fill shrinks the clip, so a stop-out still costs the budget) but not the upside give-back, since targets are fixed prices
