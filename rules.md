...
When analyzing live charts, compare price action to the **reference pattern images** included in the same request (SFP, FVG, OB setup, breaker examples from the Trading Guide).
...
3. Repeat Step 2 but on the H1 (1 hour) chart.
   ...
   3. Enter 0.5 units at 0.786 mark of H1 OB.
...
Reference images are attached in the API request as `sfp_examples.png`, `fair_value_gap_example.png`, `trading_setup.png`, `trade_off_breaker.png`. Match similar structure on the live ETH charts.
...
Four live PNG candlestick charts: **W1, D1, H4, H1** (in that order). Gray dashed lines mark recent swing high/low on each chart. Plus four reference pattern images from the Trading Guide.
